...
from leimao_model import CycleGAN


def train(train_A_dir, train_B_dir, model_dir, model_name, random_seed, validation_A_dir, validation_B_dir, output_dir, tensorboard_log_dir, num_epochs=5000):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 100 == 0:
            ...
            print('Time Elapsed for training %d epochs: %02d:%02d:%02d' % (epoch, time_elapsed_train // 3600, (time_elapsed_train % 3600 // 60), (time_elapsed_train % 60 // 1)))

        if epoch % 500 == 0:
            words = model_name.split('.')
            model_name = words[0]+'_v'+str(epoch//500 + 1)+'.'+ words[1]
        #end STEFI
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_epochs', type = int, help = 'Number epochs for training.', default = num_epochs_default)
    ...
    train(train_A_dir = train_A_dir, train_B_dir = train_B_dir, model_dir = model_dir, model_name = model_name, random_seed = random_seed, 
          validation_A_dir = validation_A_dir, validation_B_dir = validation_B_dir, output_dir = output_dir, 
          tensorboard_log_dir = tensorboard_log_dir, num_epochs = num_epochs)
